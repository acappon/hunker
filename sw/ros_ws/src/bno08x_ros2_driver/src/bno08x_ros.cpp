    ...
    : Node("bno08x_ros")
{
    ...
    running_.store(true);
    interrupt_thread_ = std::thread(&BNO08x_ROS::interrupt_thread_func, this);

    // Boost thread priority
    sched_param sp;
    sp.sched_priority = 80;
    if (pthread_setschedparam(interrupt_thread_.native_handle(),
                              SCHED_FIFO, &sp) != 0)
    {
...
void BNO08x_ROS::init_parameters()
{
    ...
    this->declare_parameter<std::string>("interrupt.gpio_chip", "gpiochip4");
    this->declare_parameter<int>("interrupt.gpio_line", 27);
...
{
    // Pi 5 uses gpiochip4 for header pins; fall back to gpiochip0 for other SBCs
    gpio_chip_ = gpiod_chip_open_by_name(gpio_chip_name_.c_str());
    if (!gpio_chip_ && gpio_chip_name_ == "gpiochip4")
    {
        RCLCPP_WARN(this->get_logger(),
                    "gpiochip4 not found, falling back to gpiochip0");
    ...
    if (!int_line_)
    {
        ...
        throw std::runtime_error("Failed to get GPIO line: " + std::to_string(gpio_line_num_));
    }
...
void BNO08x_ROS::init_sensor()
{
    ...
    try
    {
        bno08x_ = new BNO08x(comm_interface_, std::bind(&BNO08x_ROS::sensor_callback, this, std::placeholders::_1, std::placeholders::_2), this);
    }
    catch (const std::bad_alloc &e)
    {
        RCLCPP_ERROR(this->get_logger(),
